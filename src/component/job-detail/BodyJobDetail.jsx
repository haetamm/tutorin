...
const BodyJobDetail = ({ job, loading }) => {
  const { applyJobById } = useJobStore();
  const onSubmit = () => {
    applyJobById(job.id);
  };
  ...
  return (
    <>
      <div className="px-3 pt-3 lg:px-6 lg:pt-0">
        <ul className="md:flex mt-2 lg:mt-0">
          <li>
          ...
          <li className="md:list-disc md:ml-7">
            <p className="text-black text-sm hover:text-black visited:text-black">
              {job.salary}
            </p>
          </li>
        </ul>
        <div className="mt-4 mb-8 md:flex md:justify-between">
          <button
            onClick={() => !job.applied && onSubmit()}
            className={`k-w-full md:k-w-auto md:k-inline-block k-btn-primary k-btn-lg k-mb-4 md:k-mb-0 p-4 rounded mb-2 lg:mb-0 ${
              job.applied ? "bg-gray-400 cursor-not-allowed" : "bg-blue-400"
            }`}
